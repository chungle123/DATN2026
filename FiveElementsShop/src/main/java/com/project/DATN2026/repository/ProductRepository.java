...
import com.project.DATN2026.dto.Statistic.BestSellerProduct;
import com.project.DATN2026.dto.Statistic.ProductStatistic;
...
//            "p.price as price " +
    ...
    @Query(value = "SELECT p.id as idSanPham,p.code as maSanPham,p.name as tenSanPham,p.brand.name as nhanHang,p.material.name as chatLieu,p.category.name as theLoai,p.status as trangThai,p.averageRating as averageRating FROM Product p where (:maSanPham is null or p.code like CONCAT('%', :maSanPham, '%')) and " +
            "(:tenSanPham is null or p.name like CONCAT('%', :tenSanPham, '%')) and (:nhanHang is null or p.brand.id=:nhanHang) and " +
            "(:chatLieu is null or p.material.id=:chatLieu) and (:theLoai is null or p.category.id=:theLoai) and (:trangThai is null or p.status=:trangThai) and (:danhGia is null or p.averageRating >= :danhGia) and(p.deleteFlag = false)")
    ...
    @Query(value = "SELECT p.id as idSanPham,p.code as maSanPham,p.name as tenSanPham,p.brand.name as nhanHang,p.material.name as chatLieu,p.category.name as theLoai,p.status as trangThai,p.averageRating as averageRating FROM Product p where p.deleteFlag = false")
